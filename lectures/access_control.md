...
Для директорий могут назначаться права по умолчанию (default entries). Права по умолчанию назначаются всем новым файлам и поддиректориям. Для установки таких прав используется флаг `-d`: `setfacl -d -m u:user2:rwx dir`.

Автоматически создадутся дефолтные права для владельца и главной группы, как и дефолтная маска.

### Удаление прав

Для удаления прав используется флаг `-x`: 

```bash
setfacl -x u:user2:rwx dir    # Удаление конкретной записи для user2
setfacl -x g:group2 file      # Удаление конкретной записи для group2
setfacl -x m:: file           # Удаление маски
setfacl -x d:g:group2 file    # Удаление дефолтной записи для group2
```

Для удаления всех прав используется флаг `-и`: 

```bash
sudo setfacl -b my_file.txt   # Очистка всех обычных ACL прав
sudo setfacl -b my_dir        # Очистка всех обычных ACL прав и шаблонов
```
